...
const getBook = async function (req, res) {
    let searchBook = await bookModel.find().populate('author').populate('publisher')
...
const hardCoverUpdate = async function (req, res) {
    ...
    for(let i=0; i<searchPublisher.length; i++){
     if((searchPublisher[i].name == "Penguin")||(searchPublisher[i].name =="HarperCollins")){
        await bookModel.updateMany({publisher:searchPublisher[i]._id}, {$set:{isHardCover:true}})
     }}
...
const priceUpdate = async function (req, res) {
    await bookModel.updateMany({ratings:{$gt:3.5}}, {$set:{price:{$sum:10}}})
    res.send("updated")
    }
